...
module "s3_bucket" {
  source                 = "git::https://github.com/cloudposse/terraform-aws-s3-bucket.git?ref=feature/cp-34/terraform-aws-s3-bucket"
...
output "s3_user_name" {
  value       = "${local.s3_enabled == "true" && local.s3_user_enabled == "true" ? module.s3_bucket.user_name : ""}"
  description = "Normalized IAM user name"
}

output "s3_user_arn" {
  value       = "${local.s3_enabled == "true" && local.s3_user_enabled == "true" ? module.s3_bucket.user_arn : ""}"
  description = "The ARN assigned by AWS for the user"
}

output "s3_user_unique_id" {
  value       = "${local.s3_enabled == "true" && local.s3_user_enabled == "true" ? module.s3_bucket.user_unique_id : ""}"
  description = "The user unique ID assigned by AWS"
}

output "s3_access_key_id" {
  sensitive   = true
  value       = "${local.s3_enabled == "true" && local.s3_user_enabled == "true" ? module.s3_bucket.access_key_id : ""}"
  description = "The access key ID"
}

output "s3_secret_access_key" {
  sensitive   = true
  value       = "${local.s3_enabled == "true" && local.s3_user_enabled == "true" ? module.s3_bucket.secret_access_key : ""}"
  description = "The secret access key. This will be written to the state file in plain-text"
}

output "s3_bucket_arn" {
  value       = "${local.s3_enabled == "true" ? module.s3_bucket.s3_bucket_arn : ""}"
